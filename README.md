...
- `agent_core/`：智能体核心代码，包含 ReAct 循环、工具注册表、多 Agent 协作 API 和前端探索静态文件。
- `config/`：Agent 与关系配置，使用 YAML 描述。
- `docs/`：调研报告、架构说明、代码走读和结构索引。
- `experiments/`：真实实验与日志，包含诊断、场景、工具调用等实验记录。
- `memories/`：会话归档和仓库级经验沉淀。
- `tests/`：单元测试，标准库 unittest，零外部依赖。
- `.github/skills/`：可复用的技能资产，用于初始化、调研、会话总结等。
